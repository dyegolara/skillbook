...
metadata:
  ...
  upstream-path: "pstack/skills/why/SKILL.md"
  client-only:
...
Before spawning investigators, list the available MCPs from whatever environment your harness exposes. Use the available-tools map when present. Otherwise inspect the `mcps/` directory your harness exposes for enabled MCP servers.
...
Spawn each investigator with your harness's task mechanism if it has one; otherwise run it inline. Subagent config (each):
- `model`: your configured `why investigators` model, falling back to the parent model when unset
...
Spawn one synthesizer subagent with your harness's task mechanism if it has one; otherwise run the synthesis inline:

- `model`: your configured `why synthesizer` model, falling back to the parent model when unset
