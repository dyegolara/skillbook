...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles your review bot or review-automation comments. Bugbot is the default example; the watch-pr tool's review-bot tokens (`PR_REVIEW_BOT_AUTHORS`, `PR_REVIEW_BOT_BODY_TOKENS`, `PR_REVIEW_BOT_IDENTITY`, `PR_REVIEW_BOT_PASS_KEYS`) are configurable. The goal is not to ignore review-bot comments by default. The goal is to stop treating every comment as a required code change.
...
### Upstack or stack-local usage your review bot cannot see
