...
4. Verify each component against its baseline via image diff on the matching surface via the control skill. A nonzero diff is a fail. Investigate the pixel delta. Loop per component until the diff is zero, using your harness's loop or scheduled-wakeup capability when it has one (for example `/loop`); otherwise iterate manually.
