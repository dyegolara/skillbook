...
3. Explore in subagents with the `poteto-agent` definition through your harness's task mechanism when it has one, otherwise inline, and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
6. Run `node skills/pstack/poteto-mode/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill).
...
- [ ] On the operator's go, arm the program goal (your harness's goal mechanism when it has one; otherwise the recorded plan) with this exact text. "<The plan path, the PR ids in order, the verification rule, who merges, and the done condition.>"
- [ ] Read these from trunk at program start. Re-read them at every tick.
  - [ ] `git show origin/main:skills/pstack/poteto-mode/playbooks/<execution playbook>.md`
  - [ ] `git show origin/main:skills/pstack/swarm/SKILL.md`
  - [ ] `git show origin/main:<control skill path>`
  - [ ] `git show origin/main:skills/pstack/poteto-mode/playbooks/opening-a-pr.md`
  - [ ] `git show origin/main:skills/pstack/<each other leaf skill the program uses>`
- [ ] Arm the 30-minute audit tick. In a local session, a real terminal loop or scheduled wakeup when your harness has one (for example `/loop`); otherwise poll on the interval. In a cloud/background root, a cloud-sleeper wake chain. Never leave the cadence to memory.
- [ ] Use this tick prompt, verbatim. "Re-read the execution playbook from trunk and the armed program goal. Audit the operation against both and fix drift in this tick. Probe every active lane and judge progress by side effects only. Stand down a stuck lane and dispatch its replacement now. Then post a status message to the operator in chat, whether or not anything changed, with the queue table of PR, owner, state, and head SHA, the verdicts since the last tick, what merged, open operator gates, and blockers."
...
- [ ] Run your harness's slop-strip capability before each commit when it has one (for example `/deslop`); otherwise do the pass manually against the **unslop** skill. Run the **no-comments** skill before review.
- [ ] Triage every review-bot (for example Bugbot) and security-reviewer comment per `../references/bugbot-triage.md`.
- [ ] Rebase onto current trunk before babysit and again before the merge-ready report.

### Verdict and merge, for every PR

- [ ] At the merge-ready head SHA, run the swarm per `skills/pstack/swarm/SKILL.md`. One gates lane. The ten live lanes from the PR's **Verify, live** block. The perf lane from its **Verify, perf** block. One audit lane that reads the diff and the receipts and distrusts the PR body.
- [ ] Clean only when every lane is `PASS`. Findings go back to the owner. A new head gets a fresh swarm and a fresh verdict.
- [ ] <The merge or append rule from the execution playbook, with the patch-id rule from `playbooks/shipping.md`.>

### Boot recipe, for every live lane

Each live lane runs on its own cloud/background VM when your harness offers one, otherwise locally, at the PR head. Drive through `control-ui` or `control-cli` from `cursor-team-kit` when available, for example; otherwise drive the surface with the repo's own tooling.
...
<Docs to read before editing. Which PRs get `skills/pstack/how/SKILL.md` and `skills/pstack/interrogate/SKILL.md`. The trail per `skills/pstack/show-me-your-work/SKILL.md`.>
````
