...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation through your harness's task mechanism when it has one, otherwise inline, using your configured perf-issue model (falling back to your harness's default model when unset). Review the diff. Capture a post-fix trace.
