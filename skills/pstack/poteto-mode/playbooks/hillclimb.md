...
5. Loop, one hypothesis per iteration:
   - Hand the change to a subagent through your harness's task mechanism when it has one, otherwise inline, using your configured hillclimb model (falling back to your harness's default model when unset) with a tight scope. Supervise and review the diff rather than typing it (the **guard-the-context-window** principle skill). When several independent hypotheses are live, fan them to parallel subagents, each in its own worktree (the **separate-before-serializing-shared-state** principle skill).
