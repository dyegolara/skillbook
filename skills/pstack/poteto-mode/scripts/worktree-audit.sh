...
# with the legacy Cursor location as fallback. macOS only: BSD `stat -f`,
# `date -r`, and `du -sh` flags; also expects `rg` and `jq` on PATH.
slug=$(printf '%s' "$main_wt" | sed 's#^/##; s#/#-#g')
transcripts="$HOME/.agents/projects/$slug/agent-transcripts"
if [ ! -d "$transcripts" ] && [ -d "$HOME/.cursor/projects/$slug/agent-transcripts" ]; then
