...
type Forge = "gt" | "gh" | "git";

function detectForge(): Forge | null {
  if (executableOnPath("gt")) return "gt";
  if (executableOnPath("gh")) return "gh";
  if (executableOnPath("git")) return "git";
  return null;
}
...
function ghPullRequest(entry: unknown): GhPullRequest {
  if (entry === null || typeof entry !== "object") {
    throw new UserError("gh pr list returned a non-object row");
  }
  const row = entry as Record<string, unknown>;
  const number = row.number;
  if (typeof number !== "number" || !Number.isSafeInteger(number) || number <= 0) {
    throw new UserError("gh pr list returned an invalid PR number");
  }
  const state = row.state;
  if (state !== "OPEN" && state !== "CLOSED" && state !== "MERGED") {
    throw new UserError(
      `gh pr list returned an unknown PR state for PR #${number}`
    );
  }
  const headRefName = row.headRefName;
  if (typeof headRefName !== "string" || headRefName === "") {
    throw new UserError(`gh pr list returned no head branch for PR #${number}`);
  }
  const baseRefName = row.baseRefName;
  if (typeof baseRefName !== "string" || baseRefName === "") {
    throw new UserError(`gh pr list returned no base branch for PR #${number}`);
  }
  return { number, state, headRefName, baseRefName };
}

function githubPullRequests(repo: string): readonly GhPullRequest[] {
  let raw: string;
  try {
    raw = execFileSync(
      "gh",
  ...
  try {
    ...
    throw new UserError("gh pr list returned invalid JSON");
  }
  if (!Array.isArray(parsed)) {
    throw new UserError("gh pr list did not return an array");
  }
  return parsed.map((entry) => ghPullRequest(entry));
}
...
function githubFrontier(repo: string): readonly GtFrontierEntry[] {
  ...
  if (ordered.length === 0) {
    throw new UserError(
      `gh found no stacked pull requests from trunk ${trunk}; install Graphite (gt) for full stack discovery`
    );
  }
...
function resolveFrontier(repo: string): readonly FrontierPr[] {
  ...
  if (forge === "gt") {
    ...
    throw new UserError(
      "git cannot resolve the stack frontier on its own: install Graphite (gt) or the GitHub CLI (gh), or set the frontier from a clone that already has one"
    );
  } else {
    throw new UserError(
      "no supported forge found on PATH: Git (git) is required, and stack discovery needs Graphite (gt) or the GitHub CLI (gh)"
    );
  }
