...
const LANE_PATTERN = /Ten lanes on `([^`]+)` at the PR head/;
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
function configuredLaneModels() {
	const candidates = [
		process.env.PSTACK_MODELS_FILE,
		join(homedir(), ".agents", "rules", "pstack-models.md"),
	...
	for (const candidate of candidates) {
		...
		for (const line of fs.readFileSync(candidate, "utf8").split(/\r?\n/)) {
			...
			if (!match || !/lane/i.test(match[1])) continue;
			return match[2]
				.split(",")
...
const LANE_MODELS = [...new Set([...configuredLaneModels(), "inherit-parent", "auto"])];
...
for (const pr of prSections) {
	...
	if (live) {
		const liveText = [live.rest, ...live.lines.map((l) => l.text)].join("\n");
		const laneMatch = LANE_PATTERN.exec(liveText);
		if (!laneMatch) fail(live.n, `${pr.title}: Verify, live lacks "Ten lanes on \`<lane model>\` at the PR head"`);
		else if (!LANE_MODELS.includes(laneMatch[1])) {
			fail(
				live.n,
				`${pr.title}: Verify, live lanes use "${laneMatch[1]}"; configured lane models are [${LANE_MODELS.join(", ")}]`
			);
		}
