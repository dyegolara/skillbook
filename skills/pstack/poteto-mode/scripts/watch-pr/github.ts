...
const DEFAULT_REVIEW_BOT: ReviewBotConfig = {
  authors: ["bugbot"],
  identity: "cursor",
  bodyTokens: [
    "bugbot",
  ...
  passKeys: ["RUN_ID", "CURSOR_AUTOMATION_ID"],
};

function commaList(value: string | undefined, fallback: readonly string[]): readonly string[] {
...
function reviewBotConfig(env: NodeJS.ProcessEnv = process.env): ReviewBotConfig {
  const passKeys =
    env.PR_REVIEW_BOT_PASS_KEYS === undefined || env.PR_REVIEW_BOT_PASS_KEYS.trim() === ""
      ? DEFAULT_REVIEW_BOT.passKeys
      : env.PR_REVIEW_BOT_PASS_KEYS.split(",")
          .map((key) => key.trim())
          .filter((key) => key !== "");
  return {
    authors: commaList(env.PR_REVIEW_BOT_AUTHORS, DEFAULT_REVIEW_BOT.authors),
    identity: (env.PR_REVIEW_BOT_IDENTITY ?? DEFAULT_REVIEW_BOT.identity).toLowerCase(),
    bodyTokens: commaList(env.PR_REVIEW_BOT_BODY_TOKENS, DEFAULT_REVIEW_BOT.bodyTokens),
    passKeys,
  };
}
...
function isReviewBot(comment: T.ReviewComment | null): boolean {
  ...
  return (
    config.authors.some((token) => author.includes(token)) ||
    (author === config.identity &&
      config.bodyTokens.some((token) => body.includes(token)))
  );
}
...
function parseReviewThreadsFromNodes(
  nodes: readonly unknown[]
): readonly T.ReviewThread[] {
  const threads = parseReviewThreadNodes(nodes);
  ...
  return threads
    ...
    .map(({ id, firstComment }) => ({
      id,
      firstComment,
      isBugbot: isReviewBot(firstComment),
      bugbotReviewPasses: passes,
    }));
}
export function parseReviewThreadsPage(value: unknown): {
  ...
  const nodes = list(container.nodes, "reviewThreads.nodes");
  if (!("pageInfo" in container) || container.pageInfo === undefined)
    return { nodes, endCursor: null };
  const pageInfo = record(container.pageInfo, "reviewThreads.pageInfo");
  if (typeof pageInfo.hasNextPage !== "boolean")
    missing("reviewThreads.pageInfo.hasNextPage", pageInfo.hasNextPage);
  const endCursor = optionalString(
    pageInfo.endCursor,
    "reviewThreads.pageInfo.endCursor"
  );
  return {
    nodes,
    endCursor: pageInfo.hasNextPage && endCursor ? endCursor : null,
  };
}
