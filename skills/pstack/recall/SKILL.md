...
metadata:
  ...
  upstream-path: "pstack/skills/recall/SKILL.md"
  client-only:
...
Transcripts live at `~/.agents/projects/<slug>/agent-transcripts/<uuid>/<uuid>.jsonl`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/Users/you/proj` becomes `Users-you-proj`). Every line is one chat message. On older Cursor setups the same transcripts sit under `~/.cursor/projects/<slug>/agent-transcripts/`, kept as a fallback.
