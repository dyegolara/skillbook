...
description: "Periodic pass that keeps a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, at most one PR of proven corrections. Use for \"audit the verify skill\" or a periodic honesty pass."
license: MIT
metadata:
  upstream: "https://github.com/cursor/plugins/tree/main/pstack"
  upstream-commit: "f5bdd6826fd0a0d9cbc4347134c3a74a200b9d9d"
  upstream-path: "pstack/skills/maintain-verification-skill/SKILL.md"
  client-only:
...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.agents/skills/verify-*/`, falling back to your harness's project-local skill directory). Several candidates → ask which one; none → stop and point at the **create-verification-skill** skill instead of inventing a target.
