...
metadata:
  ...
  upstream-path: "pstack/skills/show-me-your-work/SKILL.md"
  client-only:
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the active workspace's `agent-transcripts/` directory (the system prompt names the path), at `~/.agents/projects/<slug>/agent-transcripts/` or the legacy `~/.cursor/projects/<slug>/agent-transcripts/` as a fallback. Don't glob across `~/.agents/projects/*/` or the legacy `~/.cursor/projects/*/`. That reads unrelated private chats. Walk the log against what actually happened:
