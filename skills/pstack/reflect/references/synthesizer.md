...
- Existing-skill-first: propose `new skill via create-skill:` only when no existing skill is a real home, the pattern recurs, and the topic deserves its own skill; otherwise author it directly following the format of neighboring skills.
...
- "your review bot flagged regex backtracking on May 2"
- "we renamed a model constant in `encodingForModel`"
...
- "skill-bundled scripts run under Node with their own lockfile, not pnpm workspace"
...
| <new pattern, no existing skill is a real home> | <draft a new skill via your harness's skill-authoring capability if it has one; otherwise author it directly> | <new skill via create-skill: <kebab-name>> |
